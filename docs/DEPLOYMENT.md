...
- **Frontend**: Deploy to Vercel (or similar). The repo root has a `package.json` whose build script runs `npm install` and `npm run build` inside `frontend/` (npm is used on Vercel to avoid pnpm registry issues). In Vercel **Project Settings > Build & Development Settings**, set **Output Directory** to `frontend/dist`. Alternatively, set **Root Directory** to `frontend` and leave Output Directory blank (Vite default is `dist`).
